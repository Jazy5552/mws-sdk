...
var pr = module.exports = function(sReport) {
  ...
  sReport.replace('/[\r]/g', ''); //Look at me, I own string now
  sReport = sReport.trim(); //Remove dumb trailing new lines
	var rows = sReport.split('\n'); //Split at newlines
	//Now we shall create a two dimensional array!
	for (var i = 0; i < rows.length; i++) {
    rows[i] = rows[i].trim(); //Damn amazon, again with the trailing spaces
	...
	for (var i = 1; i < rows.length; i++) {
		...
		for (var j = 0; j < rows[i].length; j++) {
			if (i === 1 && rows[0][j] !== undefined) { //Funky logic...
