  ...
  //console.log(pathReq);
	...
	var req = this.conn.request(options, function (res) {
	  ...
	  res.addListener('end', function() { 
		...
		else if (data.indexOf('xmlns') > -1) {
			//Finances is not returning the first <?xml... line
			//So this will look for the xmlns=... and then just parse
			//if it is there
			if(process.env.NODE_ENV === 'development') {
				console.log('------------------------------------');
				console.log('FORCE PARSING LIKE A FUCKING SAVAGE');
				console.log('------------------------------------');
			}
			//callback(data);
			parser.parseString(data);
...
function AmazonMwsRequest(options) {
	this.api = {
		path: options.path || '/',
		version: options.version || '2015-05-01' || '2009-01-01',
...
AmazonMwsRequest.prototype.set = function(param, value) {
	...
	var setValue = function(name, val) {
		params[name] = { "name" : name }; //Made a right mess of this
		//Was not working prior... We were using 
		//request.params.StartDate.value
		//So I brought this back to life Hopefully
		if (p.type == 'Timestamp') {
			params[name].value = val.toISOString();
		} else if (p.type == 'Boolean') {
			params[name].value = val ? 'true' : 'false';
		} else {
...
exports.ReportParser = require('./reportparser');
